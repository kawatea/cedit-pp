...
namespace view {
    ...
    macro_dialog::macro_dialog(Gtk::Window& window) : Gtk::Dialog(_("Macro"), window), box(Gtk::ORIENTATION_VERTICAL), before_label(_("Before expansion: ")), after_label(_("After expansion: ")) {
        ...
        before_entry.signal_changed().connect(sigc::bind<Gtk::Button*>(sigc::mem_fun(*this, &macro_dialog::on_entry_change), create_button));
        after_entry.signal_changed().connect(sigc::bind<Gtk::Button*>(sigc::mem_fun(*this, &macro_dialog::on_entry_change), create_button));
    ...
    void macro_dialog::update_macros() {
        ...
        show_all_children();
        present();
        before_entry.grab_focus();
        while (get_response()) ;
        hide();
    }
    
    void macro_dialog::on_entry_change(Gtk::Button* create_button) {
        create_button->set_sensitive(before_entry.get_text_length() > 0 && after_entry.get_text_length() > 0 && keys.count(before_entry.get_text()) == 0);
    }
